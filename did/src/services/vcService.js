...
function verifyBloodVC(vc) {
  try {
    if (vc?.id && store.isRevoked(vc.id)) return { isValid: false, reason: '취소되거나 갱신된 VC입니다.' };
...
async function initMockStore() {
  if (store.listVCs().length) return;
  const vcDatabase = [
    await issueBloodVC({
  ...
  vcDatabase.forEach(store.saveVC);
...
module.exports = {
  ...
  matchCandidates,
  initMockStore,
  getStoredVCs: store.listVCs,
  setStoredVCs: store.replaceForTest,
  saveVC: store.saveVC,
  revokeVC: store.revokeVC,
};
