...
- **데모 혈액 검사정보 분리 (2026-09-10)**: 혈액형과 Rh는
  `src/data/demoBloodProfileStore.ts`의 가상 오프체인 저장소에서 관리한다. 공개 증서 객체와
  증서 목록·상세·검증 화면에는 혈액형을 포함하지 않으며, 등록된 데모 지갑만 발급할 수 있다.
  실제 운영에서는 이 저장소를 접근 제어가 적용된 백엔드 DB로 교체해야 한다.
